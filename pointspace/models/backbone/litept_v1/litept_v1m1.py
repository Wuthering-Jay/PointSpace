...
class PointROPEAttention(PointModule):
    def __init__(
        ...
        if enable_flash:
            ...
            # follow PT-v3 non-flash path: use dynamic patch size and explicit attention
    ...
    def forward(self, point):
        ...
        if pos.shape[0] % K == 0:
            ...
            pos = pos_patch.reshape(1, -1, 3).contiguous()
        else:
            # fallback: 不做 patch-local，避免小样本崩溃
            pos = pos.reshape(1, -1, 3).contiguous()
        ...
        if not self.enable_flash:
            q = q.squeeze(0).transpose(0, 1).reshape(-1, K, H, C // H).permute(
                0, 2, 1, 3
            )
            k = k.squeeze(0).transpose(0, 1).reshape(-1, K, H, C // H).permute(
                0, 2, 1, 3
            )
...
@MODELS.register_module("LitePT-v1m1")
